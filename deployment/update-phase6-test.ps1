...
$files=@(
  'src/service-config.ts','src/network-check.tsx','src/app-updates.tsx','src/auth-page.tsx',
  'src/settings-pages.tsx','src/styles.css','vite.config.ts','src/main.tsx','public/sw.js'
)
...
try {
  ...
  Invoke-Command -Session $session -ArgumentList $stamp,$files -ScriptBlock {
    ...
    foreach($service in @('web')) {
      docker tag "business-management-test-${service}:baseline" "business-management-test-${service}:pre-phase6-$Stamp"
      if ($LASTEXITCODE -ne 0) { throw 'QA image rollback tag failed' }
    }
    docker exec business-management-test-db-1 pg_dump -U business_test -d business_management_test -Fc -f "/tmp/pre-phase6-$Stamp.dump"
    if ($LASTEXITCODE -ne 0) { throw 'QA database backup failed' }
    docker cp "business-management-test-db-1:/tmp/pre-phase6-$Stamp.dump" "$root\rollback\$Stamp\database.dump"
  ...
  Invoke-Command -Session $session -ScriptBlock {
    ...
    docker compose --project-name business-management-test --env-file .env -f compose.yaml up -d --no-deps --wait --wait-timeout 180 web 2>&1 | ForEach-Object { "$_" }
    if ($LASTEXITCODE -ne 0) { throw 'QA startup failed' }
    $ready=Invoke-RestMethod -Uri 'http://127.0.0.1:18088/api/v1/health/ready'
    if ($ready.status -ne 'ready') { throw 'QA readiness probe failed' }
    Write-Output 'PASS: QA web and business readiness endpoint'
  }
