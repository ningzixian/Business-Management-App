...
try {
  ...
  Invoke-Command -Session $session -ArgumentList $stamp,$files -ScriptBlock {
    ...
    foreach($service in @('api','web')) {
      docker tag "business-management-test-${service}:baseline" "business-management-test-${service}:pre-phase1-$Stamp"
  ...
  Invoke-Command -Session $session -ScriptBlock {
    ...
    docker compose --project-name business-management-test --env-file .env -f compose.yaml exec -T api node /app/test-verify-phase1.cjs 2>&1 | ForEach-Object { "$_" }
    if ($LASTEXITCODE -ne 0) { throw 'Phase 1 API regression failed' }
  }
