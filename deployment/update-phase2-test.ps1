...
$files=@(
  'src/business-clock.tsx','src/business-metrics.ts','src/live-reports.tsx','src/task-preview.tsx','deployment/test/verify-phase2.cjs',
...
try {
  ...
  Invoke-Command -Session $session -ArgumentList $stamp,$files -ScriptBlock {
    ...
    foreach($service in @('api','web')) {
      docker tag "business-management-test-${service}:baseline" "business-management-test-${service}:pre-phase2-$Stamp"
  ...
  Invoke-Command -Session $session -ScriptBlock {
    ...
    docker compose --project-name business-management-test --env-file .env -f compose.yaml exec -T api node /app/test-verify-phase2.cjs 2>&1 | ForEach-Object { "$_" }
    if ($LASTEXITCODE -ne 0) { throw 'Phase 2 API regression failed' }
  }
